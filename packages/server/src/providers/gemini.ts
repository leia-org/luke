...
const GEMINI_VOICES: VoiceConfig[] = [
    { id: 'Puck', name: 'Puck' },
...
export function gemini(options: GeminiProviderOptions): LukeProvider {
...
async function createGeminiConnection(
    ...
    function handleGeminiMessage(message: Record<string, unknown>) {
        ...
        if (serverContent) {
            ...
            if (modelTurn?.parts) {
                ...
                for (const part of parts) {
                    if (part.inlineData) {
                        ...
                        const audioB64 = inlineData.data as string;
                        if (audioB64) {
            ...
            if (inputTranscription?.text) {
                inputTranscriptBuffer += inputTranscription.text as string;
                // Send as partial
            ...
            if (outputTranscription?.text) {
                ...
                outputTranscriptBuffer += outputTranscription.text as string;
                onTranscriptionHandler?.({
                    role: 'assistant',
